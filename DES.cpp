...
public:
	...
	void encrypt(const string& plain_txt, const string& key)
	{
		...
		std::cout << "\n(HEX) After Parity drop: ";
		cout << Bin_to_Hex(key_firstHalf);
		std::cout << " ";
		cout << Bin_to_Hex(key_secondHalf);
		
		if (sb) {
			cout << " \n\t(bin)" << formatBin(key_firstHalf) << " " << formatBin(key_secondHalf);
		}
		...
		std::cout << "(HEX) Shift Left Round 1: ";
		std::cout << Bin_to_Hex(L_key[0]);
		std::cout << " ";
		std::cout << Bin_to_Hex(R_key[0]);
		if (sb) {
			cout << " \n\t(bin)" << formatBin(L_key[0]) << " " << formatBin(R_key[0]);
		}
		cout << endl;
		...
		for (int i = 1; i < 16; i++)
		{
			...
			std::cout << "(HEX) Shift Left Round "<<i+1<<" :";			
			std::cout << Bin_to_Hex(L_key[i]);
			std::cout << " ";	
			std::cout << Bin_to_Hex(R_key[i]);
			if (sb) {
				cout << " \n\t(bin)" << formatBin(L_key[i]) << " " << formatBin(R_key[i]);
			}
			cout << endl;
		}
		...
		for (int i = 0; i < 16; i++)
		{
			cout << "(HEX) After Pbox - Roundkey:" << i + 1 << " :";
			key_48[i] = "";
			for (int j = 0; j < 48; j++)
				key_48[i] += keys_56[i][pc_2[j] - 1]; // making 48 bits keys
			cout << Bin_to_Hex(key_48[i]);
			if (sb) {
				cout << " \n\t(bin)" << formatBin(key_48[i]);
			}
			cout << endl;
		}
		...
		cout << "(HEX) PlainTexr After Initial Permutation box:  ";
		...
		for (int i = 32; i < 64; i++)
			R += IP[i];
		cout << Bin_to_Hex(L) << " " << Bin_to_Hex(R) ;

		if (sb) {
			cout << " \n\t(bin)" << formatBin(L)<<" "<< formatBin(R);
		}
		...
		cout << "(HEX) PlainText ROUND " << 1 << " :";
		cout << Bin_to_Hex(L_32[0]) << " " << Bin_to_Hex(R_32[0]);


		if (sb) {
			cout << " \n\t(bin)" << formatBin(L_32[0]) << " " << formatBin(R_32[0]);
		}
		cout << endl;
...

		{
			...
			cout << Bin_to_Hex(L_32[i]) << " " << Bin_to_Hex(R_32[i]);

			if (sb) {
				cout << " \n\t(bin)" << formatBin(L_32[i]) << " " << formatBin(R_32[i]);
			}
			cout << endl;
		...
		cout << "Result (After Final Permutation) : " << Bin_to_Hex(encrypted_bin);

		if (sb) {
			cout << " \n\t(bin)" << formatBin(encrypted_bin);
		}
		cout << endl;
	}
